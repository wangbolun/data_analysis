...
from tools.listens_state import ListensState
from tools.read_case import ReadCase


class Test_UI:
    def __init__(self):
        self.lcm_logger_state = 0
        # 从文件中加载UI定义
    ...
    def lcm_logger(self):
        ...
        self.lcm_logger_state = 1  # 状态变更  防止重复记录
        ...
        if self.login_car == 0:
            #改变状态  进行数据记录
            self.listens_state.data_state = 1
    ...
    def kill_lcm(self):
        self.lcm_logger_state = 0  # 防止重复触发
        # 切换目录  接触LCM锁  状态绘制  窗口反馈   数据显示
        if self.login_car == 0:
            #self.listens_state.out_run(0)
            #self.updata_state(0)
            self.listens_state.data_state = 2    #状态变更
    ...
    def login_case(self):
        ...
        if self.ui.radioButton.isChecked() == True:
            ...
            self.updata()
            #控制配合车数据记录按钮
            self.listens_state = ListensState()
    ...
    def updata(self):
        def run():
            while True:
                ...
                self.listens_state.out_run()     #一直遍历状态下发
        t = Thread(target=run)
        t.start()
    ...
    def updata_vv6(self):
        def run():
            while True:
                ...
                # 一直遍历监听状态
                if self.listens_state.state == 0:
                    pass
                elif self.listens_state.state == 1 and self.lcm_logger_state != 1:
                    self.lcm_logger()
                elif self.listens_state.state == 2 and self.lcm_logger_state != 0:
                    self.kill_lcm()
                else:
                    pass
